...
{
public:
	c_linealND(int DIMin1,int DIMout1) { DIMin=DIMin1+1;DIMout=DIMout1;borra();}//ojo, dimin+1 pues hay cte a0: z=a0+a1*x+a1*y+...
	void borra();
...
private:
	...
	double S[DMAX][DMAX];
	double T[DMAX][DMAX];
	double K[DMAX][DMAX];//ctes de salida
};
...
void c_linealND::halla_coeficientes(double *kk)
{
	...
	CvMat* Mk=cvCreateMat(DIMin,DIMout,CV_64FC1);
	...
	imprimematriz(DIMout,DIMin,Mt);
	imprimematriz(DIMout,DIMin,Mk);
