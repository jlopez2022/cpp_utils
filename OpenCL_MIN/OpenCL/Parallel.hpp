...
protected:
	...
	void init_gpu(int platform_number1, int device_number1, char *kernel_filename, char **kernel_function_name, int num_kernels1);
	...
	void checkerror2(int errornum, char *texto) { checkerror2_nonstop(errornum, texto); getchar(); getchar(); }
	void checkerror2_nonstop(int errornum, char *texto) { checkError(errornum, texto); }

private:
	void init0(){kernel=nullptr;queue=nullptr;context=nullptr;context=NULL;queue=NULL;kernel=NULL;num_kernels=0;}
	void init_platform(int platform_number1,int device_number1,cl_device_id &device);
	void init_device(cl_device_id device, char *kernel_filename, char **kernel_function_name,int num_kernels1);
