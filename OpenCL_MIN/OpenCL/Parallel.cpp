...
void c_parallel::avisa_error_nonstop(int error_num1)
{
	...
	if ((error_num1>0) || (error_num1 < -60) || (i>59) )
		printf("ERROR de KERNEL desconocido = %i", error_num1);
...
void c_parallel::check_platform(void)
{
	...
	for (iPlatform = 0; iPlatform < nplatforms; iPlatform++)
	{
		...
		{   //VER: https://www.khronos.org/registry/cl/sdk/1.0/docs/man/xhtml/clGetDeviceInfo.html
			...
			cl_device_id device = devices[iDevice];
			char buffer[1024];
...
void __stdcall pfn_notify(const char *errinfo, const void *private_info, size_t cb, void *user_data) { 	fprintf(stderr, "OpenCL Error (via pfn_notify): %s\n", errinfo); }
...
{
	init0();
	cout<<"Initiating GPU. Loading kernel filename:"<<kernel_filename<< "\nPlatform number="<<platform_number1<< " device number="<<device_number1<<endl;
	for (int i=0;i<num_kernels1;i++) cout<<" kernel = " << kernel_function_name[i]<<endl; 

	num_kernels=num_kernels1;kernel=new cl_kernel[num_kernels1]; 
	cl_device_id device;
	init_platform(platform_number1,device_number1,device);
	init_device(device,kernel_filename, kernel_function_name,num_kernels1);
}

void c_parallel::init_platform( int platform_number1,int device_number1,cl_device_id &device)
{
	int status = -999;
	try
	{
		...
		cl_uint nplatforms;
		cout << "clGetPlatformIDs():" << endl;
		status = clGetPlatformIDs(0, NULL, &nplatforms); if (nplatforms <= 0) { cout << "ERROR in c_parallel::init_platform() no GPUs. Exiting" << endl; getchar(); getchar(); exit(0); }
		cl_platform_id* platforms = new cl_platform_id[nplatforms];//FALTA el delete final!!!!!!!!!!!!!!!!!!!!!
		status = clGetPlatformIDs(nplatforms, platforms, NULL);	checkerror1(status, "ERROR in c_parallel::init_platform()= clGetPlatformIDs-2 ");

		cout << "clGetDeviceIDs():" << endl;
		cl_uint ndevices;
		cl_device_id* devices;
		status = clGetDeviceIDs(platforms[platform_number1], DEVICE_TYPE_1, 0, NULL, &ndevices);	checkerror1(status, "ERROR in c_parallel::init_platform() clGetDeviceIDs ");
		if (ndevices <= 0) { cout << "ERROR in c_parallel::init_platform() no opencl devices. Exiting" << endl; getchar(); getchar(); exit(0); }
		devices = new cl_device_id[ndevices]; //FALTA el delete final!!!!!!!!!!!!!!!!!!!!!
		status = clGetDeviceIDs(platforms[platform_number1], DEVICE_TYPE_1, ndevices, devices, NULL);	checkerror1(status, "ERROR in c_parallel::init_platform() clGetDeviceIDs-2 ");
		device = devices[device_number1];
		cout <<"\nPlatform and device info:"<<endl;
		cout<< "We have "<<nplatforms<<" platforms and we have "<< ndevices   << " in the platform number "<<platform_number1<<" and we will use device number "<< device_number1 <<endl;
		if (nplatforms>1) cout << "Caution: the number of available platforms are "<< nplatforms << " and we are going to use the number "<< platform_number1 <<endl;
		if (ndevices  >1) cout << "Caution: the number of available devices   are "<< ndevices   << " in the platform number "<<platform_number1 << " and we are going to use the device number "<< device_number1 <<endl;
		cout<<endl;
		delete[] platforms;
		delete[] devices;
	}
	catch (int error)
	{
		...
		cout << "ERROR en c_parallel::init_platform(): " << endl;
		checkerror2_nonstop(status, "ERROR en c_parallel::init_platform() try catch");
		checkerror2_nonstop(error,  "ERROR en c_parallel::init_platform() try catch");
		getchar(); getchar(); exit(0);
	}
}

void c_parallel::init_device(cl_device_id device, char *kernel_filename, char **kernel_function_name,int num_kernels1)
{
	int status = -999;
	cl_program program;
	...
	try
	{
		...
		cout << "clGetDeviceInfo:CL_DEVICE_NAME: " << buffer << endl;
		status = clGetDeviceInfo(device, CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(buffer), buffer, NULL);checkerror1(status, "ERROR in c_parallel::init_device() CL_DEVICE_MAX_WORK_GROUP_SIZE ");
		cout << "clGetDeviceInfo: max work items (=max LOCAL_SIZE):"  << buffer << endl;
		status = clGetDeviceInfo(device, CL_DEVICE_NAME, sizeof(buffer), buffer, NULL); checkerror1(status, "ERROR in c_parallel::init_device() CL_DEVICE_NAME ");
		cout<<"DEVICE_NAME = " << buffer<<endl;
		status = clGetDeviceInfo(device, CL_DEVICE_VENDOR, sizeof(buffer), buffer, NULL); checkerror1(status, "ERROR in c_parallel::init_device() CL_DEVICE_VENDOR ");
		cout<<"DEVICE_VENDOR = " << buffer<<endl;
		status = clGetDeviceInfo(device, CL_DEVICE_TYPE, sizeof(buffer), buffer, NULL); checkerror1(status, "ERROR in c_parallel::init_device() CL_DEVICE_TYPE ");
		cout<<"DEVICE_TYPE = ";
		if (buffer[0] & CL_DEVICE_TYPE_CPU) cout<<" CPU";
		if (buffer[0] & CL_DEVICE_TYPE_GPU) cout<<" GPU";
		if (buffer[0] & CL_DEVICE_TYPE_ACCELERATOR) cout<<" ACCELERATOR";
		if (buffer[0] & CL_DEVICE_TYPE_DEFAULT) cout<<" DEFAULT DEVICE";
		if (buffer[0] & CL_DEVICE_TYPE_ALL) cout<<" CL_DEVICE_TYPE_ALL";
		if (buffer[0] & CL_DEVICE_TYPE_CUSTOM) cout<<" CL_DEVICE_TYPE_CUSTOM";
		cout<<endl;
		size_t max_local_size;
		clGetDeviceInfo(device, CL_DEVICE_MAX_WORK_GROUP_SIZE, 	sizeof(max_local_size), &max_local_size, NULL);
		cout<<"CL_DEVICE_MAX_WORK_GROUP_SIZE = max_local_size = " << max_local_size<<endl;

		context = clCreateContext(0, 1, &device,&pfn_notify, NULL, &status); checkerror1(status, "ERROR en c_parallel::init_device()   clCreateContext()");
		...
		propers = &prop;
		queue = clCreateCommandQueueWithProperties(context, device, propers, &status); checkerror1(status, "ERROR en c_parallel::init_device()   clCreateCommandQueue()");
		checkerror1(status, " ERROR at c_parallel::init_device()  clCreateCommandQueueWithProperties() ");
		if (!queue) 
		{
		  cout<<"FATAL ERROR at c_parallel::init_device() Cannot create OpenCL command cue: NULL\n";getchar();
		}
#endif

		size_t length;
		FILE *kernelFile = fopen(kernel_filename, "rb");
		if (!kernelFile) 		{ 			cout << "ERROR: canot open " << kernel_filename << endl; getchar(); getchar(); exit(0); 		}
        fseek(kernelFile, 0, SEEK_END);length = ftell(kernelFile);rewind(kernelFile);

        // read kernel source into buffer
        char *programBuffer = new char[length + 1];
        fread(programBuffer, sizeof(char), length, kernelFile);   programBuffer[length] = '\0';
        fclose(kernelFile);

        // create program from buffer
		cout << "Kernel filename=" << kernel_filename << " length=" << length << endl;
        program = clCreateProgramWithSource(context, 1,   (const char**) &programBuffer, &length, &status);
		checkerror1(status, "ERROR en c_parallel::init_device() clCreateProgramWithSource() Failed to create program with source!");
		...
		{
			printf("=== Create kernel with function: %s\n",kernel_function_name[i]);
	...
	{
		//std::cout << "ERROR PerformCalculationOnDevice:" << error.what() << "(" << error.err() << ")" << " ver:https://streamcomputing.eu/blog/2013-04-28/opencl-1-2-error-codes/" << std::endl;
		cout << "ERROR en c_parallel::init_device(): " << endl;
		checkerror2_nonstop(status, "ERROR en c_parallel::init_device() try catch");
		checkerror2_nonstop(error,  "ERROR en c_parallel::init_device() try catch");
		getchar(); getchar(); exit(0);
	}
