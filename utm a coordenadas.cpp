...
//Pasa de UTM a Geográficas (WGS84) y viceversa
...
#define  UTMScaleFactor  0.9996
    ...
    *
    */
    void MapLatLonToXY (double phi,double lambda,double lambda0,double &x, double &y) //salida xy
    {
...
{
	double x,y,zone;
	double lon=33.33333333333;//x  534869
	double lat=20.0;          //y  2211516

    LatLonToUTMXY (DegToRad (lat), DegToRad (lon), zone, x,y);

	//Calculo a la inversa:
	double lat2,lon2;
	int hemisferiosur=0;// Hemisferio norte:0 sur:1
    UTMXYToLatLon (x, y, zone, hemisferiosur, lat2, lon2);
