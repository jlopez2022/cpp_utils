...
long main()
{
	...
	for (i=0;i<NUM;i++)
	{
		xy1[0]=(rand() % 200);//comentar para test de velocidad
		xy1[1]=(rand() % 200);//comentar para test de velocidad
		__int16 xy3[2];
		transformada3P(tras1,xy1,xy2);//directa a coor2
		transformada3P(tras2,xy2,xy3);//inversa a coor1 de nuevo

		double x,error;
		x=xy3[0]-xy1[0];error= x*x;//comentar para test de velocidad
		x=xy3[1]-xy1[1];error+=x*x;//comentar para test de velocidad
	...
	printf("\nError medio=%lg tiempo=%lg s",sumerror1,seconds);
